...
- 编辑器里的 Tailwind 警告同样不能忽略
- 优先使用标准类名和语义尺寸
- 任意值只在确实没有标准类可表达时使用
- 提交前如果某个文件存在 Tailwind 类名警告，需要先修正再继续
...
- 内置 prompt 默认使用英文编写
- 不为同一条 LLM 指令长期维护中英文两套 prompt
- 需要语言适配时，优先把 `locale`、用户消息和运行时上下文一起传给模型
- 只有在英文 prompt 明显无法满足效果时，才单独增加特定语言 prompt
