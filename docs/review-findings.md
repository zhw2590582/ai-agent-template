...
- 收口 profile / chat settings schema 的重复定义
  - 现在 `profile-route / profile-settings / chat/server/schemas` 之间仍有多份 feature settings 结构，后续继续扩字段时最容易漂移。
- 给 `Subagents V1` 增加 delegation budget
  - 当前仍然允许一轮回复里连续多次 delegation，这是最现实的超时和卡顿来源。
- 收紧 RAG 自动触发
  - 数学题、纯推理题等无关请求仍可能误触发检索，这会直接影响运行时噪音和成本。

#### P2

- 补齐 runtime / toolset / hook 级测试
  - 重点是 `mcp / memory / models / rag / sandbox / search / subagents` 这些已经进入 V1 的能力。
- 明确 `sandbox` 剩余 access policy 的运行时消费者
  - `allowFileUpload / allowFileDownload / allowPty` 目前更多还是前置配置，不是完整能力。
- 继续补齐 `subagents` 的 telemetry 和能力边界
  - 包括更清楚的失败反馈、`rag_agent` 的独立检索能力、组件层回归。
- 让 Search 产品层逐步 provider-neutral
  - 现在 abstraction 已在 runtime 层建立，但 UI 和类型仍然明显 Tavily-first。

#### P3

- 收掉 MCP 设置页里的冗余状态
  - `testResults` 当前没有真正进入 UI 主链路。
- 明确 `AppProfile.locale / theme` 的字段语义
  - 目前更像草稿快照，而不是稳定持久化偏好。
