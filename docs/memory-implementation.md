...
当前关于 `summaryMinMessages` 的实现说明：

- 现在使用“消息数量阈值”来决定何时开始生成或更新会话摘要
- 这样做的优点是简单、稳定、便宜，也更容易解释给用户
- 但它不够精确，无法区分“很多短消息”和“少量超长消息”

后续可以评估的优化方向：

- 保持消息数量阈值不变
- 再补一个文本长度或 token 近似阈值
- 作为 summary 触发条件的补充，而不是立刻完全切到 token 计算

当前阶段不建议直接改成严格的 token 阈值，原因是：

- 当前 runtime model 可变
- tokenizer 成本和实现复杂度会上升
- 现阶段收益还不够大
...
7. summary 触发条件仍只按消息数
   - 后续可以评估是否补充文本长度或 token 近似阈值
